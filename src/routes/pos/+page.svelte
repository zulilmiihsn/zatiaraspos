...
async function loadPOSData() {
  try {
    // Load products dengan cache yang diperpanjang
    produkData = await dataService.getProducts();
    
    // Load categories dengan cache yang diperpanjang
    kategoriData = await dataService.getCategories();
    
    // Load add-ons dengan cache yang diperpanjang
    ...
    if (typeof navigator !== 'undefined' && !navigator.onLine) {
      try {
        // Coba ambil data dari cache yang sudah ada
        const cachedProducts = await dataService.getProducts();
        if (cachedProducts && cachedProducts.length > 0) {
          produkData = cachedProducts;
        }
        
        const cachedCategories = await dataService.getCategories();
        if (cachedCategories && cachedCategories.length > 0) {
          kategoriData = cachedCategories;
        }
        
        const cachedAddOns = await dataService.getAddOns();
        if (cachedAddOns && cachedAddOns.length > 0) {
          tambahanData = cachedAddOns;
        }
      } catch (cacheError) {
        console.error('Error loading cached POS data:', cacheError);
      }
    }
  }
}
  ...
  >
    ...
    <div class="flex gap-2 overflow-x-auto px-4 py-2 bg-white -mt-2 md:-mt-4 lg:-mt-6" style="scrollbar-width:none;-ms-overflow-style:none;">
      ...
      {:else if (categories ?? []).length === 0}
        ...
        <button class="flex-shrink-0 min-w-[48px] w-12 px-3 py-2 rounded-lg border font-medium text-base cursor-pointer transition-colors duration-150 mb-1 bg-pink-500 text-white border-pink-500 flex items-center justify-center" type="button" onclick={handleShowCustomItemModal} aria-label="Tambah menu kustom">
