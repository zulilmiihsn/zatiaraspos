...
<main class="flex-1 overflow-y-auto px-4 pt-2">
  <div class="px-3 py-4">
...
{#if showQrisWarning}
  <div class="fixed inset-0 z-50 flex items-end justify-center bg-black/30">
    <div class="w-full max-w-sm mx-auto bg-white rounded-t-2xl shadow-lg animate-slideUpModal p-8 pb-6 flex flex-col items-center gap-4 qris-warning-modal">
      ...
      <button class="w-full bg-pink-500 text-white font-bold rounded-lg py-3 text-base active:bg-pink-600 transition-all mt-2 warning-btn" on:click={confirmQrisChecked}>Sudah Diperiksa</button>
    </div>
  </div>
{/if}
  ...
  <div class="fixed inset-0 z-50 flex items-end justify-center bg-black/30">
    <div class="w-full max-w-sm mx-auto bg-white rounded-t-2xl shadow-lg animate-slideUpModal p-8 pb-6 flex flex-col items-center gap-4">
      ...
      <div class="flex flex-col gap-2 w-full">
        <button class="w-full bg-green-500 text-white font-bold rounded-lg py-3 text-base active:bg-green-600 transition-all" on:click={printReceipt}>Cetak Struk</button>
        <button class="w-full bg-pink-500 text-white font-bold rounded-lg py-3 text-base active:bg-pink-600 transition-all" on:click={() => { showSuccessModal = false; goto('/pos'); }}>Kembali ke Kasir</button>
      </div>
    </div>
  </div>
{/if}
