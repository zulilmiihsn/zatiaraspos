<script lang="ts">
  ...
  import Trash from 'lucide-svelte/icons/trash'; // Moved import to top
  ...
  // State variables
  let transaksiHariIni: TransaksiItem[] = [];
  let loading: boolean = true;
  let showDeleteModal: boolean = false;
  let transaksiToDelete: TransaksiItem | null = null;
  let searchKeyword: string = '';
  let filterPayment: 'all' | 'qris' | 'tunai' = 'all';
  let showDetailModal: boolean = false;
  let selectedTransaksi: TransaksiItem | null = null;
  let showDropdownPayment: boolean = false;
  ...
  async function fetchTransaksiHariIni() {
    ...
    try {
      ...
      let fetchedTransaksi: TransaksiItem[] = [];
      if (data && !error) {
        fetchedTransaksi = data.map(t => ({
          id: t.id,
      ...
      fetchedTransaksi.sort((a, b) => new Date(b.waktu).getTime() - new Date(a.waktu).getTime());
      fetchedTransaksi = fetchedTransaksi.filter(t => t.nominal && t.nominal > 0);
      
      if (searchKeyword.trim()) {
        const keyword = searchKeyword.trim().toLowerCase();
        fetchedTransaksi = fetchedTransaksi.filter(t => t.nama?.toLowerCase().includes(keyword));
      }
      
      if (filterPayment !== 'all') {
        fetchedTransaksi = fetchedTransaksi.filter(t => {
      ...
      transaksiHariIni = fetchedTransaksi; // Assign directly
    } catch (error) {
  ...
  function confirmDeleteTransaksi(trx: TransaksiItem) {
  ...
  function openDetail(trx: TransaksiItem) {
...
<div transition:fly={{ y: 32, duration: 320, easing: cubicOut }}>
  ...
  <div class="sticky top-0 z-40 bg-white shadow-sm border-b border-gray-200 flex items-center px-4 py-4 mb-0">
    ...
    <button on:click={refreshManual} class="p-2 rounded-xl bg-pink-50 hover:bg-pink-100 transition-colors ml-2" aria-label="Refresh">
  ...
  <div class="px-4 pt-3 pb-3 bg-white sticky top-[64px] z-30 space-y-3">
    <input type="text" class="w-full border border-pink-200 rounded-lg px-3 py-2.5 text-base bg-white text-gray-800 outline-none focus:border-pink-500 focus:ring-2 focus:ring-pink-100" placeholder="Cari transaksi..." bind:value={searchKeyword} on:input={fetchTransaksiHariIni} />
    <div class="flex gap-2">
      <button class="px-4 py-2 rounded-lg border text-sm font-semibold transition-all focus:outline-none {filterPayment === 'all' ? 'bg-pink-500 text-white border-pink-500' : 'bg-white text-pink-500 border-pink-200'}" on:click={() => { filterPayment = 'all'; fetchTransaksiHariIni(); }}>Semua</button>
      <button class="px-4 py-2 rounded-lg border text-sm font-semibold transition-all focus:outline-none {filterPayment === 'qris' ? 'bg-pink-500 text-white border-pink-500' : 'bg-white text-pink-500 border-pink-200'}" on:click={() => { filterPayment = 'qris'; fetchTransaksiHariIni(); }}>QRIS</button>
      <button class="px-4 py-2 rounded-lg border text-sm font-semibold transition-all focus:outline-none {filterPayment === 'tunai' ? 'bg-pink-500 text-white border-pink-500' : 'bg-white text-pink-500 border-pink-200'}" on:click={() => { filterPayment = 'tunai'; fetchTransaksiHariIni(); }}>Tunai</button>
    </div>
  </div>

  <div class="max-w-2xl mx-auto w-full pt-[2px] px-4 pb-4">
    {#if loading}
      <div class="text-center text-gray-400 py-10 min-h-64">Memuat data...</div>
    {:else if transaksiHariIni.length === 0}
      <div class="flex flex-col items-center justify-center h-64 w-full empty-state-min-height">
    ...
    {:else}
      <div class="flex flex-col gap-2">
        {#each transaksiHariIni as trx}
          <div class="bg-white rounded-xl shadow border border-gray-100 p-4 flex items-start justify-between gap-3 cursor-pointer hover:bg-pink-50 transition-colors" on:click={() => openDetail(trx)} on:keydown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); openDetail(trx); } }} role="button" tabindex="0">
            <div>
            ...
            <div class="flex flex-col items-end gap-2">
              ...
              <button class="p-2 rounded-xl bg-red-50 hover:bg-red-100 text-red-600 transition-colors shadow-md" on:click={(e) => { e.stopPropagation(); confirmDeleteTransaksi(trx); }} title="Hapus transaksi">
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div class="bg-white rounded-2xl shadow-xl max-w-xs w-full p-6 relative flex flex-col items-center animate-slideUpModal">
        ...
        <div class="flex gap-3 w-full">
          <button class="flex-1 py-2 px-4 border border-gray-300 text-gray-700 rounded-xl font-medium hover:bg-gray-50 transition-colors" on:click={() => showDeleteModal = false}>Batal</button>
          <button class="flex-1 py-2 px-4 bg-red-500 text-white rounded-xl font-medium hover:bg-red-600 transition-colors" on:click={deleteTransaksi}>Hapus</button>
        </div>
      </div>
    </div>
  {/if}
  ...
  {#if showDetailModal && selectedTransaksi}
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4 md:px-0">
      <div class="bg-white rounded-2xl shadow-2xl border border-pink-100 max-w-md w-full p-6 md:p-8 relative animate-slideUpModal flex flex-col gap-3">
        <button class="absolute top-3 right-3 p-2 rounded-full bg-gray-100 hover:bg-gray-200 shadow-sm" on:click={() => showDetailModal = false} aria-label="Tutup">
        ...
        <div class="flex flex-col gap-1 mb-2">
          ...
          <button type="button" class="w-full border-[1.5px] border-pink-200 rounded-lg px-3 py-2.5 bg-white text-pink-500 font-medium flex items-center justify-between shadow-sm hover:bg-pink-50 transition-colors" on:click={() => showDropdownPayment = true} style="user-select:none;">
        ...
        <button class="mt-2 w-full py-3 rounded-xl bg-pink-500 text-white font-bold hover:bg-pink-600 transition-colors shadow-lg shadow-pink-200/30 text-base" on:click={() => showDetailModal = false}>Tutup</button>
      </div>
    </div>
  {/if}
