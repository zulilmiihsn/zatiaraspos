<script lang="ts">
	...
	async function printStruk() {
		...
		try {
			...
			let html = `<html><body style='width:384px;font-family:monospace;color:#000;font-size:26px;line-height:1.5;margin:0;padding:0;'>`;
			html += `<div style='text-align:center;margin-bottom:16px;'>`;
			html += `<div style='font-weight:bold;font-size:30px;text-transform:uppercase;'>${pengaturan.nama_toko}</div>`;
			...
			html += `<div style='text-align:center;font-weight:bold;font-size:26px;margin-bottom:8px;'>*** CETAK ULANG ***</div>`;
			...
			html += `<table style='width:100%;font-size:26px;margin-bottom:12px;border-collapse:collapse;'><tbody>`;
			...
			html += `<table style='width:100%;font-size:26px;margin-bottom:24px;border-collapse:collapse;'><tbody>`;
			html += `<tr><td style='text-align:left;padding-bottom:4px;'>Total:</td><td style='text-align:right;font-weight:bold;font-size:30px;'>Rp${(selectedTransaksi.nominal ?? 0).toLocaleString('id-ID')}</td></tr>`;
...
<div transition:fly={{ y: 32, duration: 320, easing: cubicOut }}>
	...
	<!-- List Transaksi -->
	<div class="mx-auto w-full max-w-2xl px-4 pt-[2px] pb-24">
		{#if loading}
		...
		{:else if transaksiHariIni.length === 0}
			<div class="flex h-64 w-full flex-col items-center justify-center">
				...
				<div class="text-base font-normal text-gray-400 md:text-lg">
		...
		{:else}
			...
			<div class="flex flex-col gap-2">
				{#each transaksiHariIni as trx (trx.id)}
					<div
						class="flex cursor-pointer items-start justify-between gap-3 rounded-xl border border-gray-100 bg-white p-4 shadow-sm transition-colors hover:bg-pink-50"
