...
export const POST: RequestHandler = async ({ request }) => {
	try {
		...
		if (!username || !password || !branch) {
			return new Response(
				JSON.stringify({ success: false, message: 'Username dan password wajib diisi' }),
				{ status: 400 }
			);
		}
		...
		const { data: user, error } = await supabase
			...
			.single();
		if (error || !user) {
			return new Response(JSON.stringify({ success: false, message: 'Username tidak ditemukan' }), {
				status: 401
			});
		}
