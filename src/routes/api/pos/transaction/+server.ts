...
type StockDeductions = Map<string, { nama: string; jumlah: number }>;
type IngredientDeductions = Map<
	string,
...
export const POST: RequestHandler = async ({ request, platform, locals }) => {
	...
	const items: ComputedTransactionItem[] = [];
	...
	for (const input of normalizedInputs) {
		...
		itemNames.push(computed.product_name);
		items.push(computed);
	}
