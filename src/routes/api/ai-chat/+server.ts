...
function parseFlexibleRange(question: string): { start: string; end: string; type: 'daily' | 'weekly' | 'monthly' | 'yearly' } {
    const q = (question || '').toLowerCase();
    const now = new Date();
    const today = toYMDWita(now);
    const toYMD = (d: Date) => toYMDWita(d);

    if (q.includes('hari ini') || q.includes('today')) return { start: today, end: today, type: 'daily' };
...
export const POST: RequestHandler = async ({ request }) => {
	try {
        ...
        const totalPemasukan = pemasukan.reduce((s: number, t: any) => s + (t.amount || t.nominal || 0), 0);
        const totalPengeluaran = pengeluaran.reduce((s: number, t: any) => s + (t.amount || t.nominal || 0), 0);
        ...
        const summary = {
            ...
            pajak,
            labaBersih,
            totalTransaksi: new Set((bukuKasPos || []).map((b: any) => b.transaction_id).filter(Boolean)).size
        };
...
Rincian Pembayaran:
${Object.entries(serverReportData.pembayaran || {}).map(([k, v]: any) => `- ${k}: ${v.jumlah} trx, Rp ${v.nominal.toLocaleString('id-ID')}`).join('\n') || '- (tidak ada)'}

Jam Ramai (Top 3):
${(serverReportData.jamRamai || []).map((s: string, i: number) => `- ${i + 1}. ${s}`).join('\n') || '- (tidak ada)'}

Produk (sample): ${serverReportData.products?.slice(0, 5).map((p: any) => p.name).join(', ') || '-'}
Kategori (sample): ${serverReportData.categories?.slice(0, 5).map((c: any) => c.name).join(', ') || '-'}
