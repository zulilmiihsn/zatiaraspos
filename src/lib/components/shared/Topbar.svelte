<script lang="ts">
  ...
  // Menggunakan event listener global, Svelte akan membersihkannya saat komponen dihancurkan
...
<div class="flex items-center justify-between px-4 pt-4 pb-3 bg-white z-10 nav-transition">
  <div class="flex items-center gap-3">
    ...
    {#if isOffline}
      <span class="ml-2 px-2.5 py-1.5 rounded-full text-xs font-semibold bg-pink-100 text-pink-600 flex items-center gap-1 shadow-sm border border-pink-200 animate-fade-in offline-status-width">
    ...
    {/if}
    {#if pendingCount > 0}
      <div class="flex items-center pending-count-gap">
