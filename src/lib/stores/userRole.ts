...
export async function validateRoleWithSupabase(): Promise<'valid' | 'invalid' | 'network_error'> {
  try {
    const { data: { session }, error: sessionError } = await getSupabaseClient(get(selectedBranch)).auth.getSession();

    // Jika ada error saat mengambil sesi, kemungkinan besar masalah jaringan.
    if (sessionError) {
      console.warn('Peringatan validasi peran: Gagal mengambil sesi (kemungkinan masalah jaringan).', sessionError.message);
    ...
    if (profileError) {
      ...
      console.warn('Peringatan validasi peran: Gagal mengambil profil (kemungkinan masalah jaringan).', profileError.message);
      return 'network_error';
    }
    ...
    if (error instanceof Error) {
      console.warn('Peringatan validasi peran: Terjadi error tak terduga (kemungkinan masalah jaringan).', error.message);
    }
    return 'network_error';
  }
}
