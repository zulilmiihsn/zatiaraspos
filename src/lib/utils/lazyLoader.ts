 ...
 */
export async function preloadComponents(components: Array<{
  name: string;
  importFn: () => Promise<any>;
}>): Promise<void> {
 ...
 */
export async function preloadModules(modules: Array<{
  name: string;
  importFn: () => Promise<any>;
}>): Promise<void> {
...
export const lazyComponents = {
  // AI Chat Modal
  aiChatModal: () => lazyLoadComponent(
    () => import('$lib/components/shared/aiChatModal.svelte'),
    'aiChatModal'
  ),

  // POS Components (placeholder - adjust paths as needed)
  posGrid: () => lazyLoadComponent(
    () => import('$lib/components/shared/modalSheet.svelte'),
    'posGrid'
  ),

  posCart: () => lazyLoadComponent(
    () => import('$lib/components/shared/modalSheet.svelte'),
    'posCart'
  ),

  // Report Components (placeholder - adjust paths as needed)
  reportChart: () => lazyLoadComponent(
    () => import('$lib/components/shared/modalSheet.svelte'),
    'reportChart'
  ),

  reportTable: () => lazyLoadComponent(
    () => import('$lib/components/shared/modalSheet.svelte'),
    'reportTable'
  ),

  // Settings Components (placeholder - adjust paths as needed)
  settingsForm: () => lazyLoadComponent(
    () => import('$lib/components/shared/modalSheet.svelte'),
    'settingsForm'
  ),

  // Modal Components
  modalSheet: () => lazyLoadComponent(
    () => import('$lib/components/shared/modalSheet.svelte'),
    'modalSheet'
  ),

  pinModal: () => lazyLoadComponent(
    () => import('$lib/components/shared/pinModal.svelte'),
    'pinModal'
  )
};
...
export const lazyModules = {
  // AI Services
  aiAnalysisService: () => lazyLoadModule(
    () => import('$lib/services/aiAnalysisService'),
    'aiAnalysisService'
  ),

  autoApplyService: () => lazyLoadModule(
    () => import('$lib/services/autoApplyService'),
    'autoApplyService'
  ),

  productAnalysisService: () => lazyLoadModule(
    () => import('$lib/services/productAnalysisService'),
    'productAnalysisService'
  ),

  // Data Services
  optimizedDataService: () => lazyLoadModule(
    () => import('$lib/services/optimizedDataService'),
    'optimizedDataService'
  ),

  // Utils
  advancedCache: () => lazyLoadModule(
    () => import('$lib/utils/advancedCache'),
    'advancedCache'
  ),

  performanceMonitor: () => lazyLoadModule(
