...
export class BundleOptimizer {
   ...
   */
  private generateOptimizationSuggestions(chunkSizes: { [key: string]: number }, totalSize: number): string[] {
    const suggestions: string[] = [];

    // Check for large chunks
    Object.entries(chunkSizes).forEach(([chunk, size]) => {
      if (size > 500000) { // 500KB
        suggestions.push(`Chunk "${chunk}" is large (${Math.round(size / 1024)}KB). Consider code splitting.`);
      }
    });

    // Check total size
    if (totalSize > 2000000) { // 2MB
      suggestions.push(`Total bundle size is large (${Math.round(totalSize / 1024)}KB). Consider lazy loading.`);
    }

    // Check for vendor chunks
    if (chunkSizes['vendor'] && chunkSizes['vendor'] > 1000000) { // 1MB
...
export const bundleUtils = {
  ...
  getOptimalChunkSize(connectionSpeed: 'slow' | 'medium' | 'fast'): number {
    switch (connectionSpeed) {
      case 'slow': return 100000; // 100KB
      case 'medium': return 250000; // 250KB
      case 'fast': return 500000; // 500KB
