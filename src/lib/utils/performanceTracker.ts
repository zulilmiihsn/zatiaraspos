...
export class PerformanceTracker {
   ...
   */
  private collectNavigationMetrics(entry?: PerformanceNavigationTiming): PerformanceMetrics['navigation'] {
    const navigation = entry || performance.getEntriesByType('navigation')[0] as PerformanceNavigationTiming;
  ...
  private collectResourceMetrics(): PerformanceMetrics['resources'] {
    ...
    const totalResponseTime = resources.reduce((sum, resource) => sum + (resource.responseEnd - resource.responseStart), 0);
  ...
  private checkThresholds(metrics: PerformanceMetrics): void {
    ...
    Object.entries(this.thresholds.pageLoad).forEach(([key, threshold]) => {
      ...
      if (value > threshold) {
        this.createAlert('warning', `pageLoad.${key}`, value, threshold, 
    ...
    if (metrics.resources.totalSize > this.thresholds.resources.maxTotalSize) {
      this.createAlert('warning', 'resources.totalSize', metrics.resources.totalSize, 
    ...
    if (metrics.resources.averageResponseTime > this.thresholds.resources.maxAverageResponseTime) {
      this.createAlert('warning', 'resources.averageResponseTime', metrics.resources.averageResponseTime, 
    ...
    if (metrics.memory.usedJSHeapSize > this.thresholds.memory.maxUsedJSHeapSize) {
      this.createAlert('error', 'memory.usedJSHeapSize', metrics.memory.usedJSHeapSize, 
  ...
  getPerformanceSummary(): {
    ...
    const averagePageLoad = this.metrics.reduce((sum, m) => sum + m.pageLoad.loadComplete, 0) / this.metrics.length;
    const averageMemoryUsage = this.metrics.reduce((sum, m) => sum + m.memory.usedJSHeapSize, 0) / this.metrics.length;
