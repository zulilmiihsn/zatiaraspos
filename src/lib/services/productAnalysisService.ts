...
export class ProductAnalysisService {
    ...
    try {
      ...
      const productsWithAddOns: ProductWithAddOns[] = products.map(product => ({
        ...
        addOns: addOns.filter(addOn => 
          product.ekstra_ids.includes(addOn.id) && addOn.is_active
        )
      }));
  ...
  async generateProductPromptData(branch?: string): Promise<string> {
    ...
    const standaloneAddOns = data.addOns.filter(addOn => 
      addOn.is_active && !data.products.some(product => 
        product.ekstra_ids.includes(addOn.id)
      )
    );
