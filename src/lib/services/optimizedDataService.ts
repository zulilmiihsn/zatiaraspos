...
export class OptimizedDataService {
	...
	async executeBatchQueries(queries: BatchQuery[]): Promise<any[]> {
		...
		try {
			...
			const nonCacheablePromises = nonCacheableQueries.map((query) => query.query());
	...
	async getTransactionData(
			...
			async () => {
				...
				for (let i = 0; i < totalBatches; i += concurrency) {
					...
					for (let j = 0; j < concurrency && i + j < totalBatches; j++) {
						...
						batchPromises.push(this.fetchBatch(connection, startDate, endDate, offset, batchSize));
					}
		...
		); // 1 minute cache for transactions
	}

	/**
	 * Helper methods
	 */
	private async fetchBukuKasData(
		connection: any,
		startTime: string,
		endTime: string
	): Promise<any[]> {
	...
	private async fetchTransaksiKasirData(
		connection: any,
		startTime: string,
		endTime: string
	): Promise<any[]> {
	...
	private async getUserProfile(branch?: string): Promise<any> {
		const connection = this.getConnection(branch);
		const { data, error } = await connection.from('profil').select('*').limit(1).maybeSingle();
