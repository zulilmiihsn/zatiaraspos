...
Migrasi total Zatiaras POS dari ekosistem Supabase ke **Cloudflare (D1 + R2 + Pages + Durable Objects Realtime)**.
...
> [!CAUTION]
> Cloudflare D1 tidak mendukung fitur Realtime/WebSocket bawaan seperti Supabase. Mekanisme realtime ditulis ulang menggunakan **Cloudflare Durable Objects + WebSocket Hibernation**.
...
### 2. Realtime (Cloudflare Durable Objects)

Mengganti Supabase Realtime dengan Durable Object WebSocket hub per cabang.

#### [NEW] `src/routes/api/realtime/+server.ts`

Endpoint WebSocket yang meneruskan koneksi browser ke Durable Object room sesuai cabang.

#### [NEW] `src/lib/realtime/durableObjectClient.ts`

Menginisialisasi WebSocket browser ke `/api/realtime?branch=...`. Menyediakan listener pengganti `supabase.channel(...)`.

#### [NEW] `src/lib/server/realtimeDurableObject.js`

Durable Object hibernatable WebSocket hub untuk broadcast invalidation event ke semua device pada cabang yang sama.

#### [MODIFY] `src/lib/services/dataService.ts`

Fungsi `subscribeToRealtimeData` dimodifikasi untuk menggunakan WebSocket Durable Object. Backend API yang memutasi DB wajib mem-publish notifikasi ke Durable Object room cabang.
...
- Binding Durable Object (`REALTIME_HUB`) dan migration `new_sqlite_classes`.

#### [MODIFY] `package.json`

- `npm uninstall @supabase/supabase-js`
- `npm install drizzle-orm @aws-sdk/client-s3`
...
2. **Durable Object Realtime Test:** Membuka 2 tab browser ➡️ Menambahkan transaksi di Tab 1 ➡️ Pastikan Tab 2 UI diperbarui instan melalui WebSocket Durable Object tanpa reload.
