...
- **Global scope**: `D:/Projects/zatiaraspos/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/Projects/zatiaraspos/.claude/notes/`)
...
2. Glob `D:/Projects/zatiaraspos/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/Projects/zatiaraspos/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/Projects/zatiaraspos/.claude/notes/` — works in any directory
